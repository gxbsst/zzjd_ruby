(function () {
    ...
    angular
        ...
        .directive('validationSubmit', validationSubmitDirective)
        .directive('validationReset', validationResetDirective);
    ...
    function validationProvider() {
        ...
        this.getErrorHTML = function (message) {
            return '<p class="validation-invalid" ng-bind-html="%1"></p>'.parse(message);
        };
        ...
        this.getSuccessHTML = function (message) {
            return '<p class="validation-valid" ng-bind-html="%1"></p>'.parse(message);
        };
    ...
    validatorDirective.$inject = ['$injector'];

    function validatorDirective($injector) {
        ...
        var validFunc = function (scope, element, validMessage, validation, ctrl) {
            if ($validationProvider.showSuccessMessage) {
                validMessage = validMessage || $validationProvider.getDefaultMsg(validation).success;
                element.next().html($compile($validationProvider.getSuccessHTML(validMessage))(scope));
            } else {
                element.next().html('');
            }
            ctrl.$setValidity(ctrl.$name, true);
            if (angular.isFunction(scope.validCallback)){
                scope.validCallback.call(scope, element, scope.$eval(validMessage), validation, validMessage);
            }
        ...
        var invalidFunc = function (scope, element, validMessage, validation, ctrl) {
            if ($validationProvider.showErrorMessage) {
                validMessage = validMessage || $validationProvider.getDefaultMsg(validation).error;
                element.next().html($compile($validationProvider.getErrorHTML(validMessage))(scope));
            } else {
                element.next().html('');
            }
            ctrl.$setValidity(ctrl.$name, false);
            if (angular.isFunction(scope.invalidCallback)) {
                scope.invalidCallback.call(scope, element, scope.$eval(validMessage), validation, validMessage);
            }
    ...
    validationSubmitDirective.$inject = ['$injector'];

    function validationSubmitDirective($injector) {
    ...
    validationResetDirective.$inject = ['$injector'];

    function validationResetDirective($injector) {
